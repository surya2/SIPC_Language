...
{
  std::string value = element->getValue() ? "true" : "false";
  visitResults.push_back(value);
}
...
void PrettyPrinter::endVisit(ASTUnaryExpr *element)
{
  ...
  if (element->getOp() == "++" || element->getOp() == "--")
  {
    visitResults.push_back("(" + exprString + element->getOp() + ")");
  }
  else
  {
    visitResults.push_back("(" + element->getOp() + exprString + ")");
  }
}
...
void PrettyPrinter::endVisit(ASTArrayOfExpr *element)
{
  ...
  visitResults.pop_back();
  std::string lenString = visitResults.back();
  visitResults.pop_back();
  visitResults.push_back("[" + lenString + " of " + elementString + "]");
}
...
void PrettyPrinter::endVisit(ASTArrayRefExpr *element)
{
  ...
  visitResults.pop_back();
  std::string arrayVarString = visitResults.back();
  visitResults.pop_back();
  visitResults.push_back(arrayVarString + "[" + indexString + "]");
}
...
/*
 * For a for loop the body should be indented, but internal header.
 * Since the internal header are expressions and their visit methods never indent
 * incrementing here works.
 */
bool PrettyPrinter::visit(ASTForLoopStmt *element)
{
  indentLevel++;
  return true;
}

void PrettyPrinter::endVisit(ASTForLoopStmt *element)
{
  std::string bodyString = visitResults.back();
  visitResults.pop_back();
  ...
  {
    stepString = " by " + visitResults.back();
    visitResults.pop_back();
    endString = visitResults.back();
    visitResults.pop_back();
    startString = visitResults.back();
    visitResults.pop_back();
    varString = visitResults.back();
    visitResults.pop_back();
  }
  else
  {
  ...
  std::string forString =
      indent() + "for (" + varString + " : " + startString + " .. " + endString + stepString + ") \n" + bodyString;
  visitResults.push_back(forString);
}

/*
 * For a iterator loop the body should be indented, but internal header.
 * Since the internal header are expressions and their visit methods never indent
 * incrementing here works.
 */
bool PrettyPrinter::visit(ASTIterStmt *element)
{
  indentLevel++;
  return true;
}

void PrettyPrinter::endVisit(ASTIterStmt *element)
{
  std::string bodyString = visitResults.back();
  visitResults.pop_back();
  std::string elementString = visitResults.back();
  ...
  visitResults.push_back(iterString);
}
